 ...
 public class PinSubscriber {
     ...
     public static void main(String[] args) throws Exception {
        ...
        if (sub == null) {
         ...
         //DataReaderListenerImpl listenerb = new DataReaderListenerImpl();
 
         GuardCondition gc = new GuardCondition();
         WaitSet ws = new WaitSet();
         ws.attach_condition(gc);
         listener.set_guard_condition(gc);
         //listenerb.set_guard_condition(gc);
 
        DataReader dra = sub.create_datareader(topa,
                                            qosh.value,
                                            listener,
                                            DEFAULT_STATUS_MASK.value);
        ...
        //System.out.println(top.get_name());
         if (!reliable) {
             listener.set_expected_count(1);
             //listenerb.set_expected_count(1);
         }
