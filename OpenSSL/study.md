...
## 보안통신 (TLS/DTLS)
### TLS
* SSL(Secure Socket Layer)에 대한 인터넷 표준 버전
* TCP/IP 네트워크를 사용하는 통신에 적용됨
* 전송계층 종단간 보안과 데이터 무결성을 확보해줌
* 웹 브라우징, 전자메일, 인스턴트 메신저, VoIP같은 응용분야에 적용

### SSL
![Alt text](./img/image-23.png)
* TCP를 이용하여 신뢰할 수 있는 end-to-end 보안 서비스를 제공하기 위해 설계
  * 라이브러리 형태로 지원
* SSL Record Protocol은 다양한 상위 계층 프로토콜에 기본적인 보안 서비스 제공
* SSL 프로토콜 스펙
![Alt text](./img/image-24.png)
* 제공되는 보안 서비스
  * 인증(Authentication)
    * Client가 접속하는 서버가 신뢰할 수 있는 서버인지 혹은 허가된 사용자인지 확인
    * 전자 서명과 인증서의 검증
      * RSA, DSA와 같은 전자 서명 알고리즘 사용
  * 무결성(Integrity)
    * 해쉬 함수를 사용하여 제공
    * MAC(Message Authentication Code) 기법을 이용하여 데이터 변조 여부 확인 가능
  * 기밀성
    * 대칭키 암호화 사용
      * DES와 같은 대칭키 암호화 알고리즘을 사용하여 제공
  * 부인 봉쇄
    * 부가적인 소프트웨어를 사용하여 응용계층에서 메시지에 대한 전자서명 사용
* SSL 중요 개념
  * SSL 연결
    * 적합한 서비스 유형을 제공하는 하나의 전송
    * Peer-to-Peer 관계
    * 일시적인 연결
    * 모든 연결은 세션과 연관됨
  * SSL 세션(Session)
    * Session은 여러 연결에서 공유될 수 있는 암호학적 보안 파라미터들의 집합
    * 클라이언트와 서버 간의 결합
    * Handshake Protocol에 의해 만들어짐
  * 세션 상태는 다음 파라미터들에 의해 규정
    * 세션 식별자(Session idenfier)
      * 활동 중인 세션 혹은 재개 가능한 상태를 식별하기 위해 서버가 선택한 임의 바이트 순서
    * 상대방 인증서(Peer ceritificate)
      * 상대방의 X.509 v3 인증서
        * X.509 : 암호학에서 공개키 인증서와 인증 알고리즘의 표준 중 공개키 기반
      * NULL 값일 수 있음
    * 압축방법(Compression method)
      * 암호화를 하기 전에 데이터를 압축하는데 사용되는 알고리즘
    * 암호 규격(Cipher spec)
      * 대량의 데이터를 암호화하는 알고리즘, MAC연산을 위한 해쉬 알고리즘을 규정
    * 마스터 비밀 값(Master Secret)
      * 클라이언트와 서버가 공유하는 48 바이트 비밀 값
      * 서버와 클라이언트 난수 값, 초기화 벡터 값을 만들 때 쓰임
    * 재개 가능성
      * 세션이 새로운 연결을 개시하는데 사용될 수 있는 지의 여부를 가리키는 플래그
  * 연결 상태는 다음 파라미터들로 규정
    * 서버와 클라이언트 난수
      * 각 연결에 대하여 서버와 클라이언트가 선택하는 바이트 순서
    * 서버의 MAC 생성용 키
      * 서버가 송신 데이터의 MAC을 계산할 때 사용하는 비밀 키
    * 클라이언트의 MAC 생성용 키
      * 클라이언트가 송신하는 데이터의 MAC을 계산할 때 사용하는 비밀키
    * 서버의 암호화 키
      * 서버가 데이터를 암호화하고 클라이언트가 복호화할 때 사용하는 비밀 키
    * 클라이언트 암호화 키
      * 클라이언트가 데이터를 암호화하고 서버가 복호화 할 때 사용하는 대칭키
    * 초기화 벡터
      * CBC모드로 블럭암호가 사용될 때 각 키마다 초기화 벡터가 사용됨
    * 순차 번호
      * 각 참가자는 각 연결마다 송수신하는 메시지에 대한 독립된 순차 번호를 유지함
* SSL 구성 Protocol
  * 3개의 sub-protocol로 구성
  * Handshake Protocol
    * 인증과 키 교환을 위함
  * Record Protocol
    * 암호화된 메시지를 주고받기 위함
  * 경고 프로토콜 (Alert Protocol)
    * 프로토콜 중 에러가 발생하면 시작
* Handshake Protocol
  * 사용자가 서버를 인증하고 둘 사이에 사용될 세션키를 교환하는 과정
  * 3개의 필드로 이루어진 메시지를 주고받음
    * 종류(type) [1byte] : Handshake Protocol에서 10가지 종류의 메시지들이 있음
    * 길이(length) [3bytes] : 메시지의 길이를 나타냄
    * 내용 (content) [1byte 이상] : 파라미터들을 포함
  * 진행 단계
    * 1단계 : 보안 능력 수립
    * 2단게 : 서버인증 및 키 교환
    * 3단계 : 사용자 인증 및 키 교환
    * 4단계 : Handshake 종료
* 보안 능력 수립
  * 사용자는 서버에게 client hello 메시지를 보냄
    * 버전 : 사용자가 지원가능한 SSL 최신버전
    * 난수 : 32비트의 타임스탬프와 28바이트ㅡㅇ리 난순
    * 세션 ID, 암호 알고리즘 목록, 압축방법을 같이 보냄
  * 서버는 사용자에게 server hello 메시지를 보냄
    * 버전, 난수(사용자의 난수와 다름), 세션 ID, 암호 알고리즘, 압축방법을 같이 보냄
* 서버 인증 및 키 교환
  * 서버가 자신을 인증하기 위한 메시지와 키 교환에 필요한 정보들을 사용자에게 보냄
  * 4개의 메시지들을 보냄
    * 인증서
      * 서버는 자신의 인증서와 그 인증서를 검증할 수 있는 인증서 체인을 보냄
    * 서버키 교환
      * 선택적, 이미 인증서를 보낸 경우에는 이 메시지를 보낼 필요가 없음
    * 인증서 요구
      * 서버가 사용자에게 인증서를 요청하는 메시지
    * 종료 메시지
      * 서버가 사용자에게 2단계가 끝났음을 알리는 메시지
* 사용자 인증 및 키 교환
  * 사용자가 2단계에서 서버가 보내준 인증서를 확인하고 응답하는 과정
    * 비대칭키 알고리즘 사용
  * 다음과 같은 메시지들을 서버에게 보냄
    * 인증서
      * 2단계에서 서버가 인증서를 요구했을 때 자신의 공개키 인증서를 보내준다
        * 만약 자신의 공개키 인증서가 없으면 인증서 없음이란 메시지 보냄
      * 서버키 교환
        * 사용자는 48바이트 크기의 pre-master secret 을 생성하여 공개키로 암호화 한 뒤 서버에게 보냄
      * 인증서 검증
        * 사용자가 인증서에 해당하는 개인키를 가지고 있음을 서버가 검증하기 위해 필요
* Handshake 종료
  * 사용자는 서버에게 두개의 메시지를 보냄
    * change cipher spec
    * Finished
  * 서버는 사용자에게 다시 동일한 메시지를 보냄
    * Change cipher specs
    * Finished
  * 양쪽이 서로 상대방의 메시지를 받으면 비밀키가 제대로 만들어졌다고 판단하여 Record Protocol 단계로 넘어감
* SSL Record Protocol
  * 상위 계층에 기본적인 보안 서비스를 제공
  * Handshake protocol 이후 사용자와 서버는 세션키를 공유하며 이를 이용하여 기밀성과 무결성을 보장하는 프로토콜
  * 기밀성
    * DES 등과 같은 암호화 방식을 선택하여 이를 이용하여 보안성을 제공
  * 무결성
    * 메시지별 HMAC 이용
  * 주요 기능
    ...
    * 메시지 암호화(Encryption)
* Alert Protocol
  * 사용자나 서버가 프로토콜 진행 중에 문제를 감지하면 보냄
  * 크게 치명적인 경고와 그렇지 않은 경고로 나뉨
  * 치켱적인 경고의 경우 연결을 끊고 관련 자료 구조들을 모두 폐기
  * 경고 종류
    * Unexpected : 예상치 않은 메시지가 도착했을 때
    * Bad record MAC : 메시지에 첨부된 MAC이 잘못되었을 경우
    * Decompression failure : 압축해제가 잘못되었다는 리턴값을 출력할 경우
    * Handshake failure : 암호알고리즘이나 압축 알고리즘을 협상할 때 자신에게 맞는 알고리즘이 전혀 없을 경우
    * Illegal parameters : 파라미터 값이 지정된 범위를 벗어났거나 다른 파라미터들과의 일관성이 없을 경우
    * No certificate : 공개키 인증서를 상대방이 요청햇는데 보내줄 인증서가 없는 경우
    * Bad certificate : 검증 실패 등으로 그 인증서는 사용할 수 없는 경우
    * Unsupported certificate : 버전이 맞지 않는 등 해당 인증서 양식을 처리할 수 없는 경우
    * Certificate revoke : 철회된 인증서의 경우
    * Certificate expired : 유효기간이 지난 인증서의 경우
    * Certificate unknown : 인증서를 검증하는 과정에서 알 수 없는 에러가 발생한 경우
    * Close notify : SSL 연결을 끊기 전에 자신은 더이상 보낼 메시지가 없다는 것을 알리는 메시지
### DTLS
* TLS 프로토콜을 UDP에 적용가능하게 해주는 프로토콜
* UDP 기반의 Application들은 DTLS을 사용함으로써, 도청, 간섭, 메시지 변조 등의 공격 들을 막을 수 있음
* UDP를 사용하는 IoT에 보안성을 추가해 줄 수 있는 프로토콜로 제시되고 있음
